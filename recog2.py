...
def real_time_gesture_recognition(model_path="gesture_model.pkl", save_frames=True, save_dir="saved_frames"):
    """实时手势识别"""
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            # 每3帧保存一次
            should_save = save_frames and (frame_counter % 3 == 0)
            
            # 如果需要保存，先保存原始图像（在绘制标注之前）
            if should_save:
            ...
            if should_save:
                ...
                frame_filename = save_path / f"f{saved_frame_counter}_c{final_prediction}.jpg"
                cv2.imwrite(str(frame_filename), original_image)
                # 在图像上显示保存提示（红色圆点）
                cv2.circle(image, (image.shape[1] - 50, image.shape[0] - 50), 10, (0, 0, 255), -1)
        else:
            # 没有检测到手部
    ...
    if save_frames:
        print(f"\n保存统计:")
        print(f"  总检测帧数: {frame_counter}")
        print(f"  已保存帧数: {saved_frame_counter}")
...
if __name__ == "__main__":
    ...
    # 设置 save_frames=True 启用帧保存, False 禁用
    # 保存的帧会存储在 saved_frames/<时间戳>/ 目录下
    real_time_gesture_recognition(model_path, save_frames=True, save_dir="saved_frames")